...
def train_epoch(model, dataloader, optimizer, device, lr_scheduler, max_grad_norm=1.0):
    ...
    for step, batch in enumerate(dataloader):
        ...
        #     print(outputs)
        #     print("Sample output:", torch.argmax(outputs.hidden_states[-1][0, :20], dim=-1))
...
def main(config):
    ...
    if config.get("wandb", False):
        wandb.init(project=config.get("wandb", "gpt2-midi"), 
                   name=config.get("wandb_run_name", "fla"),
    ...
    output_dir = "/home/christian/MIDI-RWKV/src/outputs/fla"
